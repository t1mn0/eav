...
TEST(OptionArithmeticTest, Addition) {
    ...
    EXPECT_EQ(c.value_or_exception(), 30);

    fpp::Option<double> d(1.5);
    fpp::Option<double> e(2.5);
    auto f = d + e;
    EXPECT_TRUE(f.has_value());
    EXPECT_DOUBLE_EQ(f.value_or_exception(), 4.0);
}
...
TEST(OptionArithmeticTest, Subtraction) {
    ...
    auto c = a - b;
    EXPECT_TRUE(c.has_value());
    EXPECT_EQ(c.value_or_exception(), 20);
}
...
TEST(OptionArithmeticTest, Division) {
    ...
    auto c = a / b;
    EXPECT_TRUE(c.has_value());
    EXPECT_EQ(c.value_or_exception(), 4);
...
TEST(OptionArithmeticTest, CompoundOperators) {
    ...
    EXPECT_EQ(a.value_or_exception(), 15);
    
    a -= b;
    EXPECT_TRUE(a.has_value());
    EXPECT_EQ(a.value_or_exception(), 10);
    
    a *= b;
    EXPECT_TRUE(a.has_value());
    EXPECT_EQ(a.value_or_exception(), 50);
...
TEST(OptionArithmeticTest, MixedTypes) {
    ...
    EXPECT_TRUE(c.has_value());
    EXPECT_DOUBLE_EQ(c.value_or_exception(), 25.0);
}
...
TEST(OptionArithmeticTest, StringConcatenation) {
    ...
    auto c = a + b;
    EXPECT_TRUE(c.has_value());
    EXPECT_EQ(c.value_or_exception(), "Hello World");
}
